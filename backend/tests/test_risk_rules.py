...
class TestRiskRules(unittest.TestCase):
    def test_all_contracts(self):
        ...
        risk, facts, source = evaluate_risk("Payment", clauses["Payment"])
        self.assertEqual(risk, "Medium Risk")
        self.assertEqual(facts["days_to_pay"], 15)
        
        # Termination: 30 days convenience -> Low Risk
        risk, facts, source = evaluate_risk("Termination", clauses["Termination"])
        self.assertEqual(risk, "Low Risk")
        self.assertEqual(facts["notice_days"], 30)
        
        # Automatic Renewal: 12mo term, 60 day notice -> High Risk
        risk, facts, source = evaluate_risk("Automatic Renewal", clauses["Automatic Renewal"])
        ...
        risk, facts, source = evaluate_risk("Limitation of Liability", clauses["Limitation of Liability"])
        self.assertEqual(risk, "High Risk")
        ...
        risk, facts, source = evaluate_risk("Payment", clauses_c002["Payment"])
        self.assertEqual(risk, "High Risk")
        
        # Termination: Asymmetric grounds -> High Risk
        risk, facts, source = evaluate_risk("Termination", clauses_c002["Termination"])
        self.assertEqual(risk, "High Risk")
        
        # Confidentiality: Asymmetric, no duration -> High Risk
        risk, facts, source = evaluate_risk("Confidentiality", clauses_c002["Confidentiality"])
        self.assertEqual(risk, "High Risk")
        
        # IP: Vendor retains ownership, 6mo licence -> High Risk
        risk, facts, source = evaluate_risk("Intellectual Property", clauses_c002["Intellectual Property"])
        self.assertEqual(risk, "High Risk")
        
        # Automatic Renewal: Absent -> NEI
        risk, facts, source = evaluate_risk("Automatic Renewal", clauses_c002["Automatic Renewal"])
        ...
        risk, facts, source = evaluate_risk("Intellectual Property", clauses_c007["Intellectual Property"])
        self.assertEqual(risk, "Low Risk")
        
        # Liability: 12-month cap, missing carve-outs -> Medium Risk
        risk, facts, source = evaluate_risk("Limitation of Liability", clauses_c007["Limitation of Liability"])
        self.assertEqual(risk, "Medium Risk")
        
        # Confidentiality: 1 year, missing carve-outs -> High Risk
        risk, facts, source = evaluate_risk("Confidentiality", clauses_c007["Confidentiality"])
        self.assertEqual(risk, "High Risk")
