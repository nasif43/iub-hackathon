...
export default function Home() {
  const [activeTab, setActiveTab] = useState<"review" | "history" | "about" | "standards">("review");
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-8">
        {activeTab === "review" && (
          <div className="space-y-8">
            {/* Target Contract Selection & Upload Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* Target Contract Selection Panel */}
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-3 lg:col-span-2">
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center justify-between">
                  ...
                  {analyzingAll && (
                    <span className="flex items-center gap-1 text-[10px] text-blue-400 font-bold animate-pulse">
                      <Loader2 className="w-3 h-3 animate-spin" /> Batch Reviewing All Clauses
                    </span>
                  )}
                </label>
              ...
              {/* Upload Contract Form Card */}
              <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-3 col-span-1">
                ...
                <div className="space-y-2">
                  <input
                    type="text"
                    placeholder="Title (e.g. Acme SaaS Agreement)"
                  ...
                  <input
                    type="text"
                    placeholder="Parties (e.g. Acme and Client)"
                    id="new-contract-parties"
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                  />
                  <textarea
                    placeholder="Contract Content (numbered sections...)"
                    id="new-contract-text"
                    rows={2}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                  />
                  <button
                    onClick={async () => {
                      const titleEl = document.getElementById("new-contract-title") as HTMLInputElement;
                      const partiesEl = document.getElementById("new-contract-parties") as HTMLInputElement;
                      const textEl = document.getElementById("new-contract-text") as HTMLTextAreaElement;
                      if (titleEl && partiesEl && textEl && titleEl.value && partiesEl.value && textEl.value) {
                        const newC = await uploadContract(titleEl.value, partiesEl.value, textEl.value);
                        if (newC) {
                          alert(`Contract uploaded successfully!`);
                          titleEl.value = "";
                          partiesEl.value = "";
                          textEl.value = "";
                          // Reload page contracts
                          const data = await fetchContracts();
                          setContracts(data);
                          setSelectedContract(newC);
                        } else {
                          alert("Failed to upload contract.");
                        }
                      } else {
                        alert("Please fill in all upload fields.");
                      }
                    }}
                    className="w-full py-2 bg-blue-600 hover:bg-blue-500 text-slate-100 text-xs font-semibold rounded-xl transition"
                  >
                    Upload & Segment
                  </button>
                </div>
