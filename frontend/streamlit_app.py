...
def fetch_categories():
    try:
        res = requests.get(f"{BACKEND_URL}/standards", timeout=5)
...
page = st.sidebar.radio("Go to", ["Review", "Manage Standards", "Review History", "About"])
...
if page == "Review":
    ...
    with st.expander("➕ Upload New Contract", expanded=False):
        new_title = st.text_input("Contract Title:")
        new_parties = st.text_input("Parties (e.g. Northstar Solutions Ltd. and Acme Corp):")
        new_text = st.text_area("Contract Text (include numbered sections like '2.1 Payment'):")
        if st.button("Upload & Parse Contract"):
            if new_title and new_parties and new_text:
                try:
                    res = requests.post(
                        f"{BACKEND_URL}/contracts",
                        json={"title": new_title, "parties": new_parties, "raw_text": new_text},
                        timeout=10
                    )
                    if res.status_code == 200:
                        st.success(f"Contract '{new_title}' successfully uploaded and parsed!")
                        st.rerun()
                    else:
...
# --- PAGE 1.5: MANAGE STANDARDS ---
elif page == "Manage Standards":
    st.title("Manage Company Standards")
    st.subheader("Define a New Standard or Category")
    
    with st.form("new_standard_form"):
        std_id = st.text_input("Standard ID (e.g. STD-IND-01):")
        std_category = st.text_input("Category Name (e.g. Indemnification):")
        std_text = st.text_area("Company Standard Description / Guidelines:")
        submitted = st.form_submit_button("Add / Update Standard")
        
        if submitted:
            if std_id and std_category and std_text:
                try:
                    res = requests.post(
                        f"{BACKEND_URL}/standards",
                        json={"id": std_id, "category": std_category, "text": std_text},
                        timeout=5
                    )
                    if res.status_code == 200:
                        st.success(f"Standard '{std_id}' for '{std_category}' saved successfully!")
                        st.rerun()
                    else:
                        st.error(f"Failed to save standard: {res.text}")
                except Exception as e:
                    st.error(f"Error connecting to backend: {e}")
            else:
                st.warning("Please fill out all fields.")
                
    st.subheader("Existing Standards")
    try:
        res = requests.get(f"{BACKEND_URL}/standards", timeout=5)
        if res.status_code == 200:
            st.dataframe(res.json(), use_container_width=True)
    except Exception as e:
        st.error(f"Failed to fetch existing standards: {e}")
